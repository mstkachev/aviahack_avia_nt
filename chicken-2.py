...
def display_many(images, columns=4, rows=5, figure_size=(8, 8), titles=None):
	...
	plt.show()
			# cv2.imwrite(os.path.join("./testoutput/", f'{i}.jpg'), images[i-1])
...
def display_all(images, figs_per_screen=20, titles=None):
  ...
  for image in images:
    ...
    cv2.imwrite(os.path.join("./testoutput/", f'{i}.png'), (image) * 255)
    i += 1
  ...
  for i in range(int(n/figs_per_screen)):
    ...
    display_many(images[i*figs_per_screen : (i+1)*figs_per_screen], columns=columns, rows=rows, titles=titles[i*figs_per_screen : (i+1)*figs_per_screen] if titles is not None else None)

  # leftovers
  num_leftover = n % figs_per_screen
  if num_leftover != 0:
    columns, rows = closest_square_factors(num_leftover)
    display_many(images[(i+1)*figs_per_screen:], columns=columns, rows=rows, titles=titles[(i+1)*figs_per_screen:] if titles is not None else None)
